---
layout: default
title: Home
nav_order: 1
permalink: /
---

# IcePaule Project Portfolio
{: .fs-9 }

Open-Source Projekte rund um Security, Home Automation, IoT und mehr.
{: .fs-6 .fw-300 }

---

## Projekt-Kategorien

### Security & Malware Analysis
{: .text-purple-300 }

| Projekt | Beschreibung |
|:--------|:-------------|
| [IcePorge](projects/iceporge.html) | Comprehensive Malware Analysis & Threat Intelligence Stack |
| [IcePorge-Cockpit](projects/iceporge-cockpit.html) | Cockpit web management modules for CAPE and MWDB stacks |
| [IcePorge-MWDB-Stack](projects/iceporge-mwdb-stack.html) | MWDB-core with Karton orchestration for malware sample management |
| [IcePorge-MWDB-Feeder](projects/iceporge-mwdb-feeder.html) | Multi-source malware sample aggregator |
| [IcePorge-CAPE-Feed](projects/iceporge-cape-feed.html) | MalwareBazaar to CAPE to MISP automated pipeline |
| [IcePorge-CAPE-Mailer](projects/iceporge-cape-mailer.html) | CAPE Sandbox Email Integration |
| [IcePorge-Ghidra-Orchestrator](projects/iceporge-ghidra-orchestrator.html) | Automated Ghidra reverse engineering with LLM enhancement |
| [IcePorge-Malware-RAG](projects/iceporge-malware-rag.html) | LLM-powered malware analysis using RAG |
| [IceLaborVPN](projects/icelaborvpn.html) | Secure Zero-Trust Remote Access Gateway for Malware Labs |
| [IceIntelligence](projects/iceintelligence.html) | Threat Intelligence |
| [IceCrow](projects/icecrow.html) | IceCrow |
| [IceMatrix](projects/icematrix.html) | IceMatrix |
| [IceBackup](projects/icebackup.html) | IceBackup |

### Home Automation & Networking
{: .text-green-300 }

| Projekt | Beschreibung |
|:--------|:-------------|
| [IceHomeAssist](projects/icehomeassist.html) | My Home Assistant setup |
| [IceWiFi](projects/icewifi.html) | Home-WiFi setup using UniFi equipment |
| [adguard-kiosk](projects/adguard-kiosk.html) | Adguard implementation using a Raspberry Pi 3B |
| [tibberampel](projects/tibberampel.html) | Tibberampel mit einem ESP8266 |
| [IceXWiKi](projects/xwiki-stack.html) | Self-Hosted Knowledge Stack |
| [IceMailArchive](projects/icemailarchive.html) | IceMailArchive |

### Mesh & Communication
{: .text-blue-300 }

| Projekt | Beschreibung |
|:--------|:-------------|
| [Ice-MTastik](projects/ice-mtastik.html) | Meshtastic LoRa Mesh setup |
| [IceMeshCore](projects/icemeshcore.html) | MeshCore LoRa Mesh setup |

### Data & Tools
{: .text-yellow-300 }

| Projekt | Beschreibung |
|:--------|:-------------|
| [IceDataEmphasise](projects/icedataemphasise.html) | Data Emphasise |
| [IceTimereport](projects/icetimereport.html) | Time Reporting |
| [IceAI-tax-2025](projects/iceai-tax-2025.html) | IceAI-tax-2025 |
| [IceSeller](projects/iceseller.html) | IceSeller |

### Hardware & ESP32
{: .text-red-300 }

| Projekt | Beschreibung |
|:--------|:-------------|
| [esp32cam-dataset-firmware](projects/esp32cam-dataset-firmware.html) | AI Edge - Postbox mail detection |
| [followmysun](projects/followmysun.html) | Single axis adjustment for solar panel |

### Cloud & Infrastructure
{: .text-grey-dk-100 }

| Projekt | Beschreibung |
|:--------|:-------------|
| [IceHetzner](projects/icehetzner.html) | Cloud & Infrastructure |

---

[Status Dashboard](projects/status.html){: .btn .btn-outline .fs-5 .mb-4 .mb-md-0 } - Automated documentation & security scanning status
